    ...
    def updateMotorMsg(self):
        while True:
            ...
            for motor in self.motors:
                ...
                id = motor.id
                self.msg_list.motor_state[id] = msg
            time.sleep(0.001)
    
    def startSendRecv(self):
        self.send_recv_thread = threading.Thread(target=self.updateMotorMsg, daemon=True)
        self.send_recv_thread.start()
...
class UnitreeInterface(Node):
    ...
    def __init__(self):
        ...
        self.unitree_command_sub = self.create_subscription(
            ...
            1)
        self.unitree1_status_pub = self.create_publisher(LowState, 'unitree1_status', 1)
        self.unitree2_status_pub = self.create_publisher(LowState, 'unitree2_status', 1)
        self.unitree.enableallmotor()
        self.unitree2.enableallmotor()
        self.activeROSManager()
        # self.recv_timer = self.create_timer(0.004, self.recv_timer_callback)    # period need to be check
    ...
    def processRosMsg(self):
        while True:
            # unitree1_msg_list = self.unitree.getMotorMsg()
            unitree1_msg_list = self.unitree.getMotorMsg()
            unitree2_msg_list = self.unitree2.getMotorMsg()
            self.unitree1_status_pub.publish(unitree1_msg_list)
            self.unitree2_status_pub.publish(unitree2_msg_list)
            time.sleep(self.dt)
    
    def activeUnitreeInterface(self):
        self.unitree_publish_thread = threading.Thread(target=self.processRosMsg, daemon=True)
        self.unitree_publish_thread.start()
...
if __name__ == '__main__':
    ...
    unitree_interface = UnitreeInterface()
    command_dict = {
        'st': unitree_interface.activeUnitreeInterface,
    }
